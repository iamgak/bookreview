...
$blog_listing = "SELECT `blog_list`.`id`,`blog_list`.`title`, `blog_list`.`publish_date`,`blog_list`.`content`, `blog_list`.`subject`,
                    ...
                    FROM `blog_list`
                    INNER JOIN `sub_category` AS `sub_category` ON `sub_category`.`id` = `blog_list`.`sub_category`
                    INNER JOIN `category` AS `category` ON `category`.`id` = `blog_list`.`category`
                    INNER JOIN `user_listing` AS `user` ON `user`.`id` = `blog_list`.`uid`
