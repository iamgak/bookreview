...
if (!empty($_POST)) {
    ...
    if (empty($error)) {
        $check_email_exist = $link->query("SELECT `username`,`email` FROM `register` WHERE `register`.`email` = '" . _MS($_POST['email']) . "' OR `register`.`username` = '" . _MS($_POST['username']) . "'  ");
        ...
        if (empty($error)) {
            ...
            $link->query("INSERT INTO `register` (`email`,`password`,`username`,`activation_token`) VALUES ( '" . _MS($_POST['email']) . "', password('" . _MS($_POST['password']) . "'),'" . _MS($_POST['username']) . "','$activation_token')");
